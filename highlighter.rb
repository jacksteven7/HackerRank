class Highlighter
  ...
  def highlight
    result = '<p>' + @content.gsub(/\R+/, '</p> <p>') + '</p>'
    result = result.split(" ")
    @highlights.each do |highlight|
      result[highlight[:start]] = "<mark class='tooltip' style='background-color: #" + color + "'>" + result[highlight[:start]]
      result[highlight[:end]] += "<span class='tooltiptext'>#{highlight[:comment]}</span></mark>"
      p highlight[:comment]
    end
    html.sub!("$body", result.join(" "))
  end
...
highlighter = Highlighter.new(content, highlights)
p highlighter.highlight